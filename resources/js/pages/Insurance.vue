    ...
    <App>
            ...
            <template #body>
                <div class="grid grid-cols-12 gap-8">
                    <div class="col-span-6">
                        <AppInputFormat label="First Name" :error="form.errors.firstname">
                            <PrimeInputText v-model="form.firstname" placeholder="Enter First Name" />
                        </AppInputFormat>
                    </div>
                    <div class="col-span-6">
                        <AppInputFormat label="Last Name" :error="form.errors.lastname">
                            <PrimeInputText v-model="form.lastname" placeholder="Enter Last Name" />
                        </AppInputFormat>
                    </div>
                    <div class="col-span-12">
                        <AppInputFormat label="Email Address" :error="form.errors.email">
        ...
        <InsuranceLayout v-if="step === 2">
            ...
            <template #body>
                <div class="grid grid-cols-12 gap-8">
                    <div class="col-span-6">
                        <AppInputFormat label="Street Address" :required="false" :error="form.errors.street">
                            <PrimeInputText v-model="form.street" placeholder="Enter Street Address" />
                        </AppInputFormat>
                    </div>
                    <div class="col-span-6">
                        <AppInputFormat label="Ste/Apt" :required="false" :error="form.errors.ste_apt">
                            <PrimeInputText v-model="form.ste_apt" placeholder="-" />
                        </AppInputFormat>
                    </div>
                    <div class="col-span-12">
                        <AppInputFormat label="City" :error="form.errors.city">
                            <PrimeInputText v-model="form.city" placeholder="Enter City" />
                        </AppInputFormat>
                    </div>
                    <div class="col-span-12">
                        <AppInputFormat label="State" :error="form.errors.state">
                            <PrimeSelect
                                v-model="form.state"
                                :options="states"
                                option-label="name"
                    ...
                    <div class="col-span-12">
                        <AppInputFormat label="Zip Code" :error="form.errors.zipcode">
...
import InsuranceLayout from "@/components/InsuranceBox.vue";
...
const submit = () => {
    form.post(route('insurance.store'), {
        onSuccess: () => {
            form.reset()
            step.value = 0
        },
        onError: (errors) => {
            if (['firstname', 'lastname', 'email', 'phone', 'contact_preference'].some(field => field in errors)) {
                step.value = 1
            }
        },
    })
